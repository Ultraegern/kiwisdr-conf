...
(if [ "$TRUST" = "u" ] || [ "$TRUST" = "f" ]; then gpg --verify nginx-proxy-setup.sh.asc nginx-proxy-setup.sh && echo "✅ GPG verification passed. Running script..." && bash nginx-proxy-setup.sh; else read -p "⚠️ Key not fully trusted. Run script anyway? [y/N] " yn; case $yn in [Yy]*) gpg --verify nginx-proxy-setup.sh.asc nginx-proxy-setup.sh && echo "✅ GPG verification passed. Running script..." && bash nginx-proxy-setup.sh ;; *) echo "Aborted."; exit 1 ;; esac; fi)
```
